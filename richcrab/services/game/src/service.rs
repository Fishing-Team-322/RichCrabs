...
fn from_proto_settings(settings: Option<proto::richcrab::v1::RoomSettings>) -> RoomSettings {
    ...
    RoomSettings {
        player_limit: if s.player_limit == 0 { 20 } else { s.player_limit },
        visibility,
        timers: RoomTimers {
            lobby_timer_sec: if timers.lobby_timer_sec == 0 { 45 } else { timers.lobby_timer_sec },
            question_timer_sec: if timers.question_timer_sec == 0 { 30 } else { timers.question_timer_sec },
            answer_reveal_sec: if timers.answer_reveal_sec == 0 { 10 } else { timers.answer_reveal_sec },
        },
    }
}
...
impl proto::richcrab::v1::game_service_server::GameService for GameServiceImpl {
    async fn create_room(
        ...
        let room_settings = from_proto_settings(req.settings.clone());
    ...
    async fn get_room_state(
        ...
        let pin = self.room_pins.read().await.get(&room_id).cloned().unwrap_or_default();
        let invite_token = self.redis.get_value(&redis_keys::room_invite_token_key(&room_id)).await
            .map_err(|e| Status::internal(format!("failed to load room invite token: {e}")))?;
        let invite_path = invite_token.map(|token| invite_path(&token)).unwrap_or_default();
        ...
        Ok(Response::new(proto::richcrab::v1::GetRoomStateResponse {
            ...
            current_question,
            pin,
            quiz_id: Some(proto::richcrab::v1::QuizId { value: state.quiz_id }),
            owner_user_id: Some(proto::richcrab::v1::UserId { value: state.owner_user_id }),
            title: state.title,
            invite_path,
            settings: Some(to_proto_settings(&state.settings)),
        }))
    ...
    async fn list_rooms(
        ...
        for room_id in room_ids {
            ...
            let is_owner_room = snapshot.owner_user_id.as_ref().map(|owner| owner.value.as_str()) == Some(owner_user_id.as_str());
            let is_public_room = snapshot
                .settings
                .as_ref()
                .map(|s| proto::richcrab::v1::RoomVisibility::try_from(s.visibility)
                    .unwrap_or(proto::richcrab::v1::RoomVisibility::Private)
