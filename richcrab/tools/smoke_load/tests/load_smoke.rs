...
async fn smoke_load_can_run_against_live_stack() {
    let game_addr = std::env::var("SERVICE_ADDR_GAME").unwrap_or_else(|_| "http://127.0.0.1:50051".to_string());
    let join_addr = std::env::var("SERVICE_ADDR_JOIN").unwrap_or_else(|_| "http://127.0.0.1:50052".to_string());
    ...
    let room = game
        .create_room(proto::richcrab::v1::CreateRoomRequest {
            owner_user_id: Some(proto::richcrab::v1::UserId { value: owner.clone() }),
